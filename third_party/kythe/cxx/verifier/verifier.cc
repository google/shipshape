...
enum class ThunkRet { kNoException, kSolved, kInvalidProgram };
...
class Solver {
 public:
  ...
  ThunkRet UnifyTuple(Tuple *st, Tuple *tt, size_t ofs, size_t max, Thunk f) {
    if (ofs == max) return f();
    return Unify(st->element(ofs), tt->element(ofs),
                 [this, st, tt, ofs, max, &f]() {
      return UnifyTuple(st, tt, ofs + 1, max, f);
    });
  }

  ThunkRet Unify(AstNode *s, AstNode *t, Thunk f) {
    if (EVar *e = s->AsEVar()) {
      return UnifyEVar(e, t, f);
    } else if (EVar *e = t->AsEVar()) {
      return UnifyEVar(e, s, f);
      ...
      if (App *ta = t->AsApp()) {
        return Unify(sa->lhs(), ta->lhs(), [this, sa, ta, &f]() {
          return Unify(sa->rhs(), ta->rhs(), f);
        });
      }
    } else if (Tuple *st = s->AsTuple()) {
      if (Tuple *tt = t->AsTuple()) {
        if (st->size() != tt->size()) {
          return ThunkRet::kNoException;
        }
        return UnifyTuple(st, tt, 0, st->size(), f);
      }
    }
    return ThunkRet::kNoException;
  }
  ...
  ThunkRet UnifyEVar(EVar *e, AstNode *t, Thunk f) {
    ...
    if (Occurs(e, t)) {
      ...
      printer.Print(".\n");
      return ThunkRet::kInvalidProgram;
    }
    e->set_current(t);
    ThunkRet f_ret = f();
  ...
  ThunkRet MatchAtomVersusDatabase(AstNode *atom, Thunk f) {
    for (size_t fact = 0; fact < database_.size(); ++fact) {
      ThunkRet exc = Unify(atom, database_[fact], f);
      if (exc != ThunkRet::kNoException) {
        return exc;
      }
    }
    return ThunkRet::kNoException;
  ...
  ThunkRet MatchAtom(AstNode *atom, AstNode *program, Thunk f) {
        ...
        if (id->symbol() == context_.eq_id()->symbol()) {
          if (Tuple *tu = a->rhs()->AsTuple()) {
            if (tu->size() == 2) {
              ...
              return Unify(tu->element(0), tu->element(1), f);
            }
          }
        }
      }
    }
    return MatchAtomVersusDatabase(atom, f);
  }

  ThunkRet SolveGoal(AstNode *goal, Thunk f) {
    // We only have atomic goals right now.
    if (App *a = goal->AsApp()) {
      return MatchAtom(goal, nullptr, f);
    } else {
  ...
  ThunkRet SolveGoalArray(AstNode **goals, size_t cur, size_t count, Thunk f) {
    ...
    return SolveGoal(goals[cur], [this, goals, cur, count, &f]() {
      return SolveGoalArray(goals, cur + 1, count, f);
    });
  }

  bool Solve() {
    ThunkRet exn = SolveGoalArray(context_.parser()->goals().data(), 0,
                                  context_.parser()->goals().size(),
                                  []() { return ThunkRet::kSolved; });
    return exn == ThunkRet::kSolved;
  }
...
Verifier::Verifier(bool trace_lex, bool trace_parse)
    : parser_(this, trace_lex, trace_parse),
      ...
      builtin_location_(&builtin_location_name_, 0, 0) {
...
void Verifier::DumpErrorGoal(size_t index) {
  FileHandlePrettyPrinter printer(stderr);
  if (index >= parser_.goals().size()) {
    printer.Print("(invalid index)\n");
    return;
  }
  auto *goal = parser_.goals()[index];
  ...
  if (goal_end.filename && *goal_end.filename != *kStandardIn &&
    ...
    printed_goal =
        PrintFileSection(f, goal_begin.line, goal_begin.column - 1,
                         goal_end.line, goal_end.column - 1, &printer);
    fclose(f);
  }
  if (!printed_goal) {
    goal->Dump(symbol_table_, &printer);
...
void Verifier::AssertSingleFact(std::string *database, unsigned int fact_id,
  ...
  yy::location loc(database, fact_id, 0);
  AstNode **values = (AstNode **)arena_.New(sizeof(AstNode *) * 5);
  values[0] =
