...
exports.commands = {
  ...
  run: new Command(function(engine, args) {
    var targetId = args.shift();
    var target = engine.resolveTarget(targetId);
    var executable = target.rule.getExecutable(target);
    if (!executable) {
      console.error('ERROR: "' + targetId + '" is not an executable target');
      process.exit(1);
    }
    engine.ninjaCommand(rule.kinds.BUILD, [targetId], true, function() {
      // TODO(schroederc): drop campfire_lock before executing binary
      child_process.spawn(executable.id, args, {stdio: 'inherit'})
