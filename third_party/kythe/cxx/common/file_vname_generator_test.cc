...
TEST(FileVNameGenerator, LookupGroups) {
  ...
  EXPECT_EQ(
      ...
      generator
          .LookupBaseVName(
               "campfire-out/bin/third_party/kythe/java/some/path/A.jar!/some/path/A.class")
          .DebugString());
  EXPECT_EQ(
      kythe_java_vname.DebugString(),
      generator.LookupBaseVName(
                    "third_party/kythe/java/com/google/devtools/kythe/util/KytheURI.java")
          .DebugString());
...
TEST(FileVNameGenerator, ActualConfigTests) {
  ...
  test_file.set_path(
      "third_party/kythe/cxx/extractor/testdata/extract_verify_std_string_test.cc");
  EXPECT_EQ(
