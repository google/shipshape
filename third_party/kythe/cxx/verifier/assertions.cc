...
AstNode *AssertionParser::CreateAnchorSpec(const yy::location &location) {
  ...
  EVar *new_evar = new (verifier_.arena()) EVar(location);
  std::pair<EVar *, std::string> record(new_evar, location_spec_stack_.back());
  unresolved_locations_.push_back(record);
  location_spec_stack_.pop_back();
  AppendGoal(verifier_.MakePredicate(
      location, verifier_.fact_id(),
      {new_evar, verifier_.empty_string_id(), verifier_.empty_string_id(),
       verifier_.kind_id(), verifier_.IdentifierFor(location, "anchor")}));
  return new_evar;
}
...
bool AssertionParser::ResolveLocations(const yy::location &end_of_line,
  ...
  for (auto &record : unresolved_locations_) {
    EVar *evar = record.first;
    std::string &token = record.second;
    ...
    AppendGoal(verifier_.MakePredicate(
        ...
        {evar, verifier_.empty_string_id(), verifier_.empty_string_id(),
         ...
         verifier_.IdentifierFor(location, line_start + col + token.size())}));
  }
...
void AssertionParser::AppendGoal(AstNode *goal) { goals_.push_back(goal); }
