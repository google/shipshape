...
There may be up to one positional parameter specified that ends in .kindex.
If one exists, no other positional parameters may be specified, nor may an
additional input parameter be specified. Input will be read from the index file.
...
Examples:
  ...
  // Check to see if we're meant to read from a .kindex file.
  std::string kindex_suffix = ".kindex";
  std::string kindex_file;
  for (const auto &arg : final_args) {
    if (EndsWith(arg, kindex_suffix)) {
      CHECK_EQ(2, final_args.size())
          ...
          << "from an index file.";
      CHECK_EQ("-", FLAGS_i)
          << "No other input is allowed when reading from an index file.";
      kindex_file = arg;
    }
  }
